...
def find_neighbors(S, D):
    """ Find a pair of neighbors which minimizes n_ij as defined by Saitou and Nei """
    n, _ = D.shape

    min = float('inf')
    pair = None
    for i in range(n):  # since computation is symmetric we only run through half of the matrix
        for j in range(i, n):
            if i != j:
                n_ij = D[i, j] - (r(i, S, D) + r(j, S, D))
                if n_ij < min:
                    min = n_ij
                    pair = (i, j)
...
def rapid_neighbor_joining(D, taxa):
    ...
    while len(S) > 3:
        ...
        # 1. Select i, j directly while computing entries that were originally stored in N
        i, j = find_neighbors(S, D)
        print("--------------------------")
        print("Deleting indices " + str(i) + " (" + S[i] + ") " + "and " + str(j)  + " (" + S[j] + ") " + "from D.")
...
def main():
    ...
    nj_tree = rapid_neighbor_joining(D, taxa)
